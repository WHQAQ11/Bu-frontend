      ...
      <div className="grid md:grid-cols-4 gap-6">
        <MysticalAura className="bg-midnight-800/40 backdrop-blur-sm rounded-xl p-6 border border-primary-500/20 text-center">
          <div className="space-y-3">
            ...
            <p className="text-3xl font-bold text-golden-400">{userStats?.total || 0}</p>
        ...
        <MysticalAura className="bg-midnight-800/40 backdrop-blur-sm rounded-xl p-6 border border-primary-500/20 text-center">
          <div className="space-y-3">
            ...
            <p className="text-3xl font-bold text-mystical-teal">{userStats?.recentCount || 0}</p>
      ...
      {divinationHistory.length === 0 ? (
        <MysticalAura className="bg-midnight-800/40 backdrop-blur-sm rounded-xl p-12 border border-primary-500/20 text-center">
          <div className="space-y-4">
            ...
            <h3 className="text-xl font-semibold text-midnight-100">暂无占卜记录</h3>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-midnight-900/50 rounded-lg p-4">
                  <div className="flex items-center space-x-3">
                    <div className="w-10 h-10 bg-gradient-to-br from-mystical-purple to-mystical-indigo rounded-full flex items-center justify-center">
                      <span className="text-lg font-bold">{selectedRecord.result.name}</span>
                    </div>
                    <div>
                      ...
                      <p className="text-midnight-400 text-sm">
                        {selectedRecord.result.upperTrigram}上{selectedRecord.result.lowerTrigram}下
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {selectedRecord.aiInterpretation && (
                  <div className="bg-mystical-teal/10 rounded-lg p-4 border border-mystical-teal/30">
                    <h4 className="text-mystical-teal font-medium mb-2">AI解读</h4>
  ...
  const renderStats = () => (
    <div className="space-y-8">
      ...
      <MysticalAura className="bg-midnight-800/40 backdrop-blur-sm rounded-xl p-8 border border-primary-500/20">
        <h3 className="text-xl font-semibold text-midnight-100 mb-6">占卜方法分布</h3>
        <div className="space-y-4">
          {Object.entries(userStats?.byMethod || {}).map(([method, count]) => (
            <div key={method} className="space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-midnight-200">{getMethodName(method)}</span>
      ...
      {/* 热门问题类型 */}
      <MysticalAura className="bg-midnight-800/40 backdrop-blur-sm rounded-xl p-8 border border-primary-500/20">
        <h3 className="text-xl font-semibold text-midnight-100 mb-6">热门问题类型</h3>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={item.category} className="flex items-center space-x-3 bg-midnight-700/50 rounded-lg p-3">
              <span className="text-2xl">{item.icon}</span>
              <div className="flex-1">
                <p className="text-midnight-200">{getCategoryName(item.category)}</p>

  ...
  return (
    ...
    >
      <g transform={`rotate(${rotationAngle}, ${center}, ${center})`}>
        ...
        {/* 白色鱼眼 */}
        <circle cx={center} cy={center - taiChiRadius / 2} r={taiChiRadius / 6} fill="white" />

        {/* 黑色鱼眼 */}
        <circle cx={center} cy={center + taiChiRadius / 2} r={taiChiRadius / 6} fill="black" />

        {/* 阴阳鱼的小圆点 */}
        {/* 白色部分中的小黑点 */}
        <circle cx={center} cy={center - taiChiRadius / 2} r={taiChiRadius / 20} fill="black" />

        {/* 黑色部分中的小白点 */}
        <circle cx={center} cy={center + taiChiRadius / 2} r={taiChiRadius / 20} fill="white" />

        {/* 外圆边框 */}
        <circle cx={center} cy={center} r={taiChiRadius} fill="none" stroke="black" strokeWidth="1" />
      </g>
    </svg>
  );
};
  ...
  return (
    <g>
      {pattern.map((yao, index) => {
        ...
        if (yao === 1) { // 阳爻
          return <rect key={index} x={-yaoWidth / 2} y={y} width={yaoWidth} height={yaoHeight} fill="black" />;
        } else { // 阴爻
          const brokenWidth = yaoWidth * 0.45;
          return (
            <g key={index}>
              <rect x={-yaoWidth / 2} y={y} width={brokenWidth} height={yaoHeight} fill="black" />
              <rect x={yaoWidth / 2 - brokenWidth} y={y} width={brokenWidth} height={yaoHeight} fill="black" />
            </g>
          );
        }
      })}
      ...
      <g>
        {trigramsData.map((trigram, index) => {
          ...
          return (
            <g key={trigram.name}>
              {/* 绘制卦象符号，并让它"脚朝里" */}
              <g transform={`translate(${trigramX}, ${trigramY}) rotate(${angle})`}>
